...
suite_filter_options = ("dimensions: 2")  # without filtering, a suite has instance_indices 1-15
...
for batch_counter, problem in enumerate(suite):  # this loop may take hours or days...
    ...
    while evalsleft() > 0 and not problem.final_target_hit:
        ...
        if 11 < 3:  # add solver to investigate here
            ...
            output = mocmaes.run(problem, propose_x0())
        elif fmin is EGO.EGO:
            if irestart == 0:
                X = propose_x0()
                Y = problem(X)

                ego = EGO.EGO(len(X), np.reshape(X, (-1, 1)), np.reshape(Y, (-1, 1)))
                ego.train()

            Xnew, EI = ego.next()
            Ynew = problem(np.reshape(Xnew, -1))
            output = Ynew

            ego.X = np.vstack((ego.X, np.reshape(Xnew, (-1, 1))))
            ego.Y = np.vstack((ego.Y, np.reshape(Ynew, (-1, 1))))
